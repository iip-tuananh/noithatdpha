...
class ProjectCategoryController extends Controller
{
	...
	protected $route = 'ProjectCategory';
	...
	public function nestedSort(Request $request)
    {
        if ($request->ajax()) {
            ...
            foreach($readbleArray as $row){
                ...
                $i++;
                DB::table('project_categories')->where('id',$row['id'])->update(['parent_id' => $row['parentID'], 'sort_order' => $i, 'level' => $level]);
            }
			$json->success = true;
    ...
    public function addToHomePage(Request $request) {
        $category = ProjectCategory::query()->find($request->cate_id);
